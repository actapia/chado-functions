create or replace function public.recurs_featureloc_slice(bigint, bigint, bigint)
       returns setof featureloc
       language sql
as $function$with recursive rec_features as (
select * from featureloc fl where srcfeature_id = $1 and fl.fmax >= least($2,$3) and fl.fmin <= greatest($2,$3)
union all
